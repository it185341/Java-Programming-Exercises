public class MyUtils {
    ...
    // Merge sort for students by last name
    public static void mergeSort(Student [] F){
        //If the array length is 1 or less, it's already sorted
        if(F.length > 1){
            Student [] firstHalf = new Student [F.length /2 ];
            System.arraycopy(F,0,firstHalf,0,F.length/2); 
            mergeSort(firstHalf); // Recursively sort the first half
            // Calculate length of the second half
            int secondHalfLength = F.length - F.length/2;
            Student [] secondHalf = new Student[secondHalfLength];
            System.arraycopy(F, F.length/2, secondHalf,0, secondHalfLength);
            mergeSort(secondHalf); // Recursively sort the second half
            merge(firstHalf,secondHalf, F); // Merge the two sorted halves back into the original array
        }
    }
    // Merge two sorted arrays into one sorted array by last name
    public static void merge(Student [] list1, Student [] list2, Student [] temp){
        ...
        while(current1 < list1.length && current2 <list2.length){
            if(list1[current1].getLsName().compareTo(list2[current2].getLsName())<0)
